...
const TaskListTable = ({tableData}) => {
    ...
    const getPriorityBadgeColr = (priority) => {
        switch(priority) {
    ...
    console.log(tableData,'<_-tableData')
            ...
            <tbody>
                {
                    tableData.map(task => {
                        return (<tr key={task._id} className='border-t border-gray-200'>
                            <td className="mx-4 pt-[16px] pb-[16px] text-gray-700 text-[13px] line-clamp-1 overflow-hidden">{task.title}</td>
                            <td className="px-4 pb-[16px]">
                                <span className={`px-2 py-1 text-xs rounded inline-block ${getStatusBadgeColor(task.status)}`}>{task.status}</span>
                            </td>
                            <td className="px-4 pb-[16px]">
                                <span className={`px-2 py-1 text-xs rounded inline-block ${getPriorityBadgeColr(task.priority)}`}>{task.priority}</span>
                            </td>
                            <td className="px-4 pt-[16px] pb-[16px] text-gray-700 text-[13px] text-nowrap hidden md:table-cell">
                                {
